        ...
        if _fit_btn or _resume_pending:
          ...
          try:
            ...
            if retry_indices and (channel_medians or all_k_vals):
                ...
                from pass2_helpers import retry_one_well
                import time as _time_mod
                for idx in retry_indices:
                    ...
                    _well_pos_r = _get_well_pos(sample_name)
                    _rox_r = st.session_state.get('rox_by_well', {}).get(_well_pos_r)
                    _ch_thresh_r = channel_thresholds.get(ch, global_threshold)

                    status_text.text(
                        f"Pass 2: {sample_name}  ch={ch}  "
                        f"k≈{priors['k']:.3f}  P0≈{priors['P0']:.2g}"
                    )
                    _retry_t0 = _time_mod.perf_counter()
                    results_list[idx] = retry_one_well(
                        result, priors, cycles,
                        metadata=wm, rox=_rox_r,
                        channel_threshold=_ch_thresh_r,
                        first_fit_cycle=first_fit_cycle,
                        cycles_before_max=cycles_before_max,
                        cycles_after_max=cycles_after_max,
                        auto_truncate=auto_truncate,
            ...
            # Phase-1 unified gates. run_batch.run_quality_gates is the
            # canonical implementation — driven by DEFAULT_GATES from
            # config.py so tuning experiments can sweep alternate thresholds.
            from run_batch import run_quality_gates as _run_quality_gates
            _run_quality_gates(results_list, cycles)
